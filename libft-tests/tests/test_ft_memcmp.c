...
void test_ft_memcmp(void)
{
	...
	if (ft_memcmp(pattern_a, pattern_c, 0) != 0) nomalloc_printf("%s %s\n", __func__, "test 1 fail: wrong rezult");
	if (ft_memcmp(pattern_a, pattern_b, sizeof(pattern_a)) != 0) nomalloc_printf("%s %s\n", __func__, "test 2 fail: wrong rezult");
	if (ft_memcmp(pattern_a, pattern_c, sizeof(pattern_a)) <= 0) nomalloc_printf("%s %s\n", __func__, "test 3 fail: wrong rezult");
	if (ft_memcmp(pattern_a, pattern_d, sizeof(pattern_a)) >= 0) nomalloc_printf("%s %s\n", __func__, "test 4 fail: wrong rezult");
}
